...
export default function ConcreteColumnCalc() {
  ...
  const sideNum = parse(sideLength);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    saveRoom({
      name: roomName.trim(),
      length: columnShape === "round" ? diaNum : sideNum,
      width: columnShape === "round" ? diaNum : sideNum,
      height: hNum,
      geometryType: "volume",
    });
    setRoomName("");
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-8 select-none">
      <div className="md:col-span-7 flex flex-col gap-6">
        <Card>
          <div className="flex justify-between items-center border-b border-neutral-100 dark:border-neutral-800 pb-4 mb-4">
            <h3 className="font-bold text-sm uppercase tracking-wider text-neutral-800 dark:text-neutral-200">Column Parameters</h3>
          ...
          <div className="grid grid-cols-2 gap-2 mb-6">
            <button type="button" onClick={() => setColumnShape("round")} className={`border rounded-md py-2.5 text-xs font-semibold transition-all ${columnShape === "round" ? "border-black bg-black text-white dark:border-white dark:bg-white dark:text-black" : "border-neutral-200 text-neutral-600 hover:border-neutral-800 dark:border-neutral-800 dark:text-neutral-400"}`}>Round Column</button>
            <button type="button" onClick={() => setColumnShape("square")} className={`border rounded-md py-2.5 text-xs font-semibold transition-all ${columnShape === "square" ? "border-black bg-black text-white dark:border-white dark:bg-white dark:text-black" : "border-neutral-200 text-neutral-600 hover:border-neutral-800 dark:border-neutral-800 dark:text-neutral-400"}`}>Square Column</button>
          </div>

          <div className="flex flex-col gap-4">
            {columnShape === "round" ? (
              <Input label={unitSystem === "imperial" ? "Diameter (inches)" : "Diameter (cm)"} type="number" inputMode="decimal" autocomplete="off" value={diameter} onChange={(e) => setDiameter(e.target.value)} placeholder="e.g. 12" />
            ) : (
              <Input label={unitSystem === "imperial" ? "Side Length (inches)" : "Side Length (cm)"} type="number" inputMode="decimal" autocomplete="off" value={sideLength} onChange={(e) => setSideLength(e.target.value)} placeholder="e.g. 12" />
            )}
            <div className="grid grid-cols-2 gap-4">
              <Input label={unitSystem === "imperial" ? "Height (feet)" : "Height (meters)"} type="number" inputMode="decimal" autocomplete="off" value={height} onChange={(e) => setHeight(e.target.value)} placeholder="e.g. 8" />
              <Input label="Number of Columns" type="number" inputMode="numeric" autocomplete="off" value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="e.g. 4" />
            </div>
            <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          <h4 className="font-bold text-sm uppercase tracking-wider text-neutral-800 dark:text-neutral-200 mb-4">Save Column Set</h4>
          <form onSubmit={handleSave} className="flex gap-2 items-end">
            <div className="flex-grow">
              <Input label="Save As" type="text" autocomplete="off" value={roomName} onChange={(e) => setRoomName(e.target.value)} placeholder="e.g. Porch Columns" />
            </div>
      ...
      <div className="md:col-span-5 flex flex-col gap-6">
        <Card className="bg-black text-white dark:bg-neutral-950 dark:border-neutral-800 flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-xs uppercase tracking-widest text-neutral-400 dark:text-neutral-500 mb-6">Column Volume Output</h3>
            <div className="flex flex-col gap-6 mb-8">
              <div>
                ...
                <span className="text-xs text-neutral-400 block mt-1 tabular-nums">{volWithWaste.toFixed(1)} cu ft total</span>
              </div>
              <div>
            ...
            <tbody className="tabular-nums">
              {(["80lb", "60lb", "50lb", "40lb"] as const).map((s) => (
                <tr key={s} className="border-b border-neutral-50/50 dark:border-neutral-900/50">
                  ...
                  <td className="py-2 text-right font-bold text-neutral-950 dark:text-white">{[bags80, bags60, bags50, bags40][["80lb", "60lb", "50lb", "40lb"].indexOf(s)]}</td>
                </tr>
              ))}
            </tbody>
          </table>
