    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-8 select-none">
      <div className="md:col-span-7 flex flex-col gap-6">
        <Card>
          <div className="flex justify-between items-center border-b border-neutral-100 dark:border-neutral-800 pb-4 mb-4">
            <h3 className="font-bold text-sm uppercase tracking-wider text-neutral-800 dark:text-neutral-200">Wall Parameters</h3>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <Input label={unitSystem === "imperial" ? "Wall Length (ft)" : "Wall Length (m)"} type="number" inputMode="decimal" autocomplete="off" value={length} onChange={(e) => setLength(e.target.value)} placeholder="e.g. 10" />
            <Input label={unitSystem === "imperial" ? "Wall Height (ft)" : "Wall Height (m)"} type="number" inputMode="decimal" autocomplete="off" value={height} onChange={(e) => setHeight(e.target.value)} placeholder="e.g. 8" />
          </div>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <Input label={unitSystem === "imperial" ? "Thickness (inches)" : "Thickness (cm)"} type="number" inputMode="decimal" autocomplete="off" value={thickness} onChange={(e) => setThickness(e.target.value)} placeholder="e.g. 8" />
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <Input label="Standard Doors (21 sq ft each)" type="number" inputMode="numeric" autocomplete="off" value={doorCount} onChange={(e) => setDoorCount(e.target.value)} placeholder="0" helperText="Subtracted from wall area" />
            <Input label="Standard Windows (12 sq ft each)" type="number" inputMode="numeric" autocomplete="off" value={windowCount} onChange={(e) => setWindowCount(e.target.value)} placeholder="0" helperText="Subtracted from wall area" />
          </div>

          <div className="border-t border-neutral-100 dark:border-neutral-800 pt-4">
            <label className="text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1.5 block">Bag Size for Output</label>
            <div className="grid grid-cols-4 gap-2">
              {(["40lb", "50lb", "60lb", "80lb"] as const).map((s) => (
                <button key={s} type="button" onClick={() => setBagSize(s)} className={`border rounded-md py-2 text-xs font-semibold font-mono transition-all ${bagSize === s ? "border-black bg-black text-white dark:border-white dark:bg-white dark:text-black" : "border-neutral-200 text-neutral-600 hover:border-neutral-800 dark:border-neutral-800 dark:text-neutral-400"}`}>{s}</button>
              ))}
            </div>
          </div>
        </Card>
        ...
        <Card>
          <h4 className="font-bold text-sm uppercase tracking-wider text-neutral-800 dark:text-neutral-200 mb-4">Save Wall Project</h4>
          <form onSubmit={handleSave} className="flex gap-2 items-end">
            <div className="flex-grow">
              <Input label="Save As" type="text" autocomplete="off" value={roomName} onChange={(e) => setRoomName(e.target.value)} placeholder="e.g. Basement Wall" />
            </div>
      ...
      <div className="md:col-span-5 flex flex-col gap-6">
        <Card className="bg-black text-white dark:bg-neutral-950 dark:border-neutral-800 flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-xs uppercase tracking-widest text-neutral-400 mb-6">Wall Volume Output</h3>
            <div className="flex flex-col gap-6 mb-8">
              <div>
                <span className="text-xs font-medium text-neutral-400 block mb-1">Gross Wall Area</span>
                <div className="flex items-baseline gap-2 tabular-nums">
                  <span className="text-2xl font-bold tracking-tight text-neutral-200">{grossArea.toFixed(1)}</span>
                  <span className="text-sm text-neutral-400">{unitSystem === "imperial" ? "sq ft" : "sq m"}</span>
                </div>
                {(doors > 0 || windows > 0) && (
                  <span className="text-xs text-neutral-400 block mt-1">Net after openings: {netArea.toFixed(1)} {unitSystem === "imperial" ? "sq ft" : "sq m"}</span>
                )}
              </div>
              <div>
              ...
              <div>
                ...
                <div className="flex items-baseline gap-2 tabular-nums">
                  ...
                  <span className="text-xs text-neutral-400">(~{(weight / 2000).toFixed(2)} tons)</span>
                </div>
              </div>
            </div>
          </div>
