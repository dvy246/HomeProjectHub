...
export default function BudgetPlanView() {
  ...
  if (loading) {
    return <div className="text-center py-8 text-sm text-[var(--fg-muted)]">Loading...</div>;
  }
  ...
  if (!plan) {
    return (
      <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] p-10 text-center">
        ...
        <p className="text-sm text-[var(--fg-secondary)] mb-6">This budget plan could not be found or the URL is invalid.</p>
        <a href="/renovate/plans/" className="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium rounded-lg bg-[var(--accent)] text-[var(--accent-fg)] hover:bg-[var(--accent-hover)] transition-all">Back to Budget Plans</a>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      {editingPlan ? (
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] p-5 flex flex-col gap-4">
          <div>
            <label className="text-sm font-medium block mb-1.5" htmlFor="edit-plan-name">Plan Name</label>
          ...
          <div>
            <label className="text-sm font-medium block mb-1.5" htmlFor="edit-plan-desc">Description</label>
          ...
          <div>
            <label className="text-sm font-medium block mb-1.5" htmlFor="edit-plan-status">Status</label>
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={handleSavePlanDetails} className="px-4 py-2 text-sm font-medium rounded-lg bg-[var(--accent)] text-[var(--accent-fg)] hover:bg-[var(--accent-hover)] transition-all">Save</button>
            <button type="button" onClick={() => setEditingPlan(false)} className="px-4 py-2 text-sm font-medium rounded-lg border border-[var(--border-strong)] text-[var(--fg)] hover:border-[var(--border-hover)] transition-all">Cancel</button>
          </div>
        </div>
      ) : (
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] p-5">
          ...
          <div className="grid grid-cols-3 gap-4 mb-3">
            <div className="bg-[var(--bg-subtle)] rounded-lg p-3">
              <p className="text-[11px] text-[var(--fg-muted)] uppercase tracking-wider font-medium">Total Budget</p>
              <p className="text-xl font-bold text-[var(--fg)] mt-1">${totalBudget.toLocaleString()}</p>
            </div>
            <div className="bg-[var(--bg-subtle)] rounded-lg p-3">
              <p className="text-[11px] text-[var(--fg-muted)] uppercase tracking-wider font-medium">Spent</p>
              <p className={`text-xl font-bold mt-1 ${isOver ? "text-red-500" : "text-[var(--fg)]"}`}>${totalActual.toLocaleString()}</p>
            </div>
            <div className="bg-[var(--bg-subtle)] rounded-lg p-3">
              <p className="text-[11px] text-[var(--fg-muted)] uppercase tracking-wider font-medium">Remaining</p>
          ...
          <p className="text-[11px] text-[var(--fg-muted)] mt-1.5">{pctUsed}% of budget used &middot; {plan.items.length} line item{plan.items.length !== 1 ? "s" : ""}</p>
        </div>
      )}

      {/* Line Items */}
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-[var(--fg)]">Line Items</h3>
        <button
          type="button"
      ...
      {showAddForm && (
        <form onSubmit={handleAddItem} className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] p-5 flex flex-col gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium block mb-1" htmlFor="item-category">Category</label>
            ...
            <div>
              <label className="text-xs font-medium block mb-1" htmlFor="item-name">Item Name</label>
              <input id="item-name" type="text" value={itemName} onChange={(e) => setItemName(e.target.value)} className="w-full text-sm bg-[var(--bg-inset)] border border-[var(--border)] rounded-lg h-10 px-3 text-[var(--fg)] focus:outline-none" placeholder="e.g. 80lb Concrete Bags" />
            </div>
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
            <div>
              <label className="text-xs font-medium block mb-1" htmlFor="item-qty">Quantity</label>
            ...
            <div>
              <label className="text-xs font-medium block mb-1" htmlFor="item-unit">Unit</label>
            ...
            <div>
              <label className="text-xs font-medium block mb-1" htmlFor="item-price">Unit Price ($)</label>
            ...
            <div>
              <label className="text-xs font-medium block mb-1" htmlFor="item-actual">Actual Cost ($)</label>
              <input id="item-actual" type="number" step="0.01" min="0" value={itemActual} onChange={(e) => setItemActual(e.target.value)} className="w-full text-sm bg-[var(--bg-inset)] border border-[var(--border)] rounded-lg h-10 px-3 text-[var(--fg)] focus:outline-none" placeholder="Optional" />
            </div>
          </div>
          <div>
            <label className="text-xs font-medium block mb-1" htmlFor="item-notes">Notes</label>
            <input id="item-notes" type="text" value={itemNotes} onChange={(e) => setItemNotes(e.target.value)} className="w-full text-sm bg-[var(--bg-inset)] border border-[var(--border)] rounded-lg h-10 px-3 text-[var(--fg)] focus:outline-none" placeholder="Optional notes or supplier name" />
          </div>
          <div className="flex items-center justify-between pt-2 border-t border-[var(--border)]">
            <p className="text-xs text-[var(--fg-muted)]">
              {itemQty && itemUnitPrice ? `Budget: $${(parseFloat(itemQty || "0") * parseFloat(itemUnitPrice || "0")).toFixed(2)}` : ""}
            </p>
            <div className="flex gap-2">
              <button type="button" onClick={resetForm} className="px-4 py-2 text-xs font-medium rounded-lg border border-[var(--border-strong)] text-[var(--fg)] hover:border-[var(--border-hover)] transition-all">Cancel</button>
      ...
      {plan.items.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[var(--border)] p-8 text-center">
          <p className="text-sm text-[var(--fg-muted)]">No line items yet. Add materials, labor, and other costs to build your budget.</p>
        </div>
      ) : (
              ...
              <div key={item.id} className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] p-4">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex gap-1">
                    <button type="button" onClick={() => handleEditItem(item)} className="p-1 rounded hover:bg-[var(--bg-muted)] text-[var(--fg-muted)] hover:text-[var(--fg)] transition-colors" aria-label="Edit item">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                      </svg>
                    </button>
                    <button type="button" onClick={() => handleDeleteItem(item.id)} className="p-1 rounded hover:bg-red-500/10 text-[var(--fg-muted)] hover:text-red-500 transition-colors" aria-label="Delete item">
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div className="grid grid-cols-4 gap-3 text-xs">
                  <div>
                    <span className="text-[var(--fg-muted)]">Qty:</span> {item.quantity} {item.unit}
                  </div>
                  <div>
                    <span className="text-[var(--fg-muted)]">@</span> ${item.unitPrice.toFixed(2)}
                  </div>
                  <div>
                    <span className="text-[var(--fg-muted)]">Budget:</span> ${item.totalBudget.toFixed(2)}
                  </div>
                  <div>
