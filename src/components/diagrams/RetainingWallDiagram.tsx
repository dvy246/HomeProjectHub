...
export default function RetainingWallDiagram({ length, height, baseWidth, unitSystem }: Props) {
  ...
  return (
    <svg viewBox={`0 0 ${viewW} ${viewH}`} className="diagram-svg w-full h-auto" role="img" aria-label={`Retaining wall: ${l}×${h}`}>
      <defs>
      ...
      <line x1={pad - sbw * 0.3} y1={groundY} x2={pad + sl + sbw * 0.3} y2={groundY} stroke="var(--border-strong)" strokeWidth="0.75" strokeDasharray="4 3" className="diagram-dim" />

      <line x1={pad} y1={pad + sh + sbw + 8} x2={pad + sl} y2={pad + sh + sbw + 8} stroke="var(--fg-muted)" strokeWidth="1" markerStart={`url(#arr-${id})`} markerEnd={`url(#arr-${id})`} className="diagram-dim" />
      <text x={pad + sl / 2} y={pad + sh + sbw + 22} textAnchor="middle" fill="var(--fg)" fontSize="13" fontWeight="600" className="diagram-label">{l.toFixed(1)}</text>
      <text x={pad + sl / 2} y={pad + sh + sbw + 36} textAnchor="middle" fill="var(--fg-muted)" fontSize="10" className="diagram-label">Length ({unit})</text>

      <line x1={pad + sl + 8} y1={pad} x2={pad + sl + 8} y2={groundY} stroke="var(--fg-muted)" strokeWidth="1" markerStart={`url(#arr-rev-${id})`} markerEnd={`url(#arr-${id})`} className="diagram-dim" />
      <text x={pad + sl + 10} y={pad + sh / 2 + 4} textAnchor="start" fill="var(--fg)" fontSize="13" fontWeight="600" className="diagram-label">{h.toFixed(1)}</text>
