...
export default function VinylFenceDiagram({ length, height, numSections, unitSystem }: Props) {
  ...
  return (
    <svg viewBox={`0 0 ${viewW} ${viewH}`} className="w-full h-auto diagram-svg" role="img" aria-label={`Fence: ${l}×${h}, ${n} sections`}>
      <defs>
      ...
      {Array.from({ length: n }, (_, i) => {
        ...
        return (
          <DiagramPart key={i} title={`Fence section ${i + 1}`}>
            <g>
              <rect x={x + 1} y={pad} width={sectionW - 2} height={sh} fill="var(--accent)" fillOpacity="0.05" stroke="var(--border-strong)" strokeWidth="0.75" rx="1" className="diagram-part" />
              <line x1={x + sectionW * 0.25} y1={pad} x2={x + sectionW * 0.25} y2={pad + sh} stroke="var(--border-strong)" strokeWidth="0.5" opacity="0.3" className="diagram-dim" />
              <line x1={x + sectionW * 0.75} y1={pad} x2={x + sectionW * 0.75} y2={pad + sh} stroke="var(--border-strong)" strokeWidth="0.5" opacity="0.3" className="diagram-dim" />
            </g>
          </DiagramPart>
        );
      })}
